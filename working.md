...
5. **Ops Board "Cần chú ý" mở rộng**: gom checklist quá hạn + NCC báo không nhận + sự cố mở + chờ duyệt huỷ; icon ⛔🚨🛑 cạnh mã đơn (`obIssueIcons`); card ⛔ dashboard bấm → Ops Board lọc sẵn (`gotoOpsAttention`).
6. **Sổ tay hướng dẫn `/huongdan`** (public/huongdan.html, link ❓ cuối sidebar, mở tab mới): bắt đầu/PWA/đổi pass, quy trình tour A→Z 6 bước, việc theo 6 vai trò, cổng NCC 4 bước, trang khách (/tracuu /danhgia), FAQ 7 câu. Trang tĩnh không chứa dữ liệu.

**`npm test` → 199 pass, 0 fail.** Verify Chrome: /huongdan render đẹp; Ops Board lọc "Cần chú ý" ra đúng tour có dịch vụ bị NCC từ chối kèm icon ⛔.
